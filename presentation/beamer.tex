...
\vspace{2em}
One caveat is that the result is not $U \ket{\psi}$ but $PU \ket{\psi}$, where $P$ is some Pauli operation (on each qubit) depending on the measurement outcome\cite{jozsa2006introduction}.
...
\begin{frame}
  \begin{columns}
    \begin{column}{.5\linewidth}
      Note that the numbering on the nodes is arbitrary:

      \begin{gather*}
        \Qcircuit @C=1em @R=1.3em{
          \smClstr{1}&\smClstr{2}\qw&\smClstr{3}\qw}
          =
          \Qcircuit @C=1em @R=1.3em{
          \smClstr{3}&\smClstr{2}\qw&\smClstr{1}\qw}
      \end{gather*}

      This is immediately apparent when looking at the actual state:
      {
      \begin{gather*}
        \frac{\ket{+}\ket{+}\ket{+}+\ket{-}\ket{-}\ket{-}}{\sqrt{2}}
      \end{gather*}
      }%

      Notice this state is symmetric -- `numbering' the qubits is unnecessary in this case


    \end{column}
    \begin{column}{.5\linewidth}
      This symmetry can make it easy to see how these states will be built.\\
      Consider an input of $\ket{+++}$. When this is run through a $C_Z$ gate on qubits 1 and 2 produces the state:

      {
      \begin{gather*}
        \frac{ (\ket{0}\ket{+} + \ket{1}\ket{-}) \ket{+}}{\sqrt{2}}
      \end{gather*}
      }%

      Which is now obviously equal to the state:

      {
      \begin{gather*}
        \frac{ (\ket{+}\ket{0} + \ket{-}\ket{1}) \ket{+}}{\sqrt{2}}
      \end{gather*}
      }%
...
\vspace{2em}
This begs the question: \textit{How many dimensions are necessary?}
\end{frame}
...
\begin{frame}
   ...
   \begin{columns}
	 ...
	 \begin{column}{.7\linewidth}
	   \begin{figure}\centering\includegraphics[scale=0.4]{figures/opticalclusterstate}\end{figure}
	 \end{column}
	...
	This generates a square cluster state. Two pairs of entangle photons are created and then separate pairs are entangled. 
	 \begin{gather*}
	  ...
	  \end{gather*}
	  The optical components act as the gates. 
	\end{column}
 
   \end{columns}
   \cite{prevedel2007experimental}
\end{frame}
...
\begin{frame}
    ...
    Text about polynomial time classical simulation of linear cluster chain operations, and necessity of at least 2-D cluster states for true quantum speed up of certain algorithms. This separates product-state preparation from entangling operations.
  \end{block}
